...
// correctly for USPTO, DoW, and DoC alike. The two entries that name an org
// tier read it from the tenant (`tierLabels`) rather than hardcoding Commerce's
// word — a tooltip that says "bureau" on an Army instance is exactly the leak
// the rest of this glossary avoids.
//
// The lookup KEYS are structural and never move: `<GlossaryTerm
// term="crossBureauRationalization">` call sites reference them, and
// `department-dashboard-data.ts`'s KPI cards carry them as `glossary` values.
// Only `term` and `definition` resolve through the tenant.
...
/** One tenant's glossary. Keys are fixed; only `term`/`definition` vary. */
export function getGlossary(tenant: TenantConfig = getTenant()): Record<GlossaryTermKey, GlossaryEntry> {
  ...
  return {
    ...
    awaitingBureauSignOff: {
      term: `Awaiting ${unit} sign-off`,
      definition:
        `This use case has been approved or rejected, but no record yet shows which ${unit} official signed off on that decision or when.`,
    },
    advisory: {
      term: "Advisory",
      definition:
...
/**
 * The active tenant's glossary, resolved once at module load — the same
 * deployment-constant `NEXT_PUBLIC_TENANT` assumption `lib/fieldRegistry.ts`
 * makes. Kept so `components/launchpad/glossary-term.tsx` and the other
 * consumers that hold no tenant need no change; pass a tenant to
 * `getGlossary()` when you need another org's wording.
 */
export const GLOSSARY: Record<GlossaryTermKey, GlossaryEntry> = getGlossary()
