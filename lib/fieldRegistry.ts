...
export function getFieldRegistry(tenant: TenantConfig = getTenant()): FieldDefinition[] {
  ...
  return [
    ...
    },
    {
      ...
      label: `Affected ${tiers.unitPlural}`,
      description: `Which ${tiers.unitPlural.toLowerCase()}, processes, or groups the problem affects (multi-select).`,
...
/**
 * The active tenant's registry, resolved once at module load.
 *
 ...
 * instead — that's the path tests exercise other tenants through.
 */
export const FIELD_REGISTRY: FieldDefinition[] = getFieldRegistry()

function byKey(registry: FieldDefinition[]): Record<string, FieldDefinition> {
...
/** Key -> definition for one tenant's registry. */
export function getFieldRegistryByKey(tenant: TenantConfig = getTenant()): Record<string, FieldDefinition> {
  return byKey(getFieldRegistry(tenant))
}

// Convenience lookup by field key — O(1) access during isFieldEnabled checks.
export const FIELD_REGISTRY_BY_KEY: Record<string, FieldDefinition> = byKey(FIELD_REGISTRY)

// Convenience: which fields belong to a given step.
export function fieldsForStep(step: number, tenant: TenantConfig = getTenant()): FieldDefinition[] {
  return getFieldRegistry(tenant).filter((f) => f.step === step)
}

// Convenience: which fields belong to a given phase.
export function fieldsForPhase(phase: 1 | 2 | 3 | 4 | 5, tenant: TenantConfig = getTenant()): FieldDefinition[] {
  return getFieldRegistry(tenant).filter((f) => f.phase === phase)
}
 ...
 *
 * Filters the active-tenant snapshot (`FIELD_REGISTRY`) rather than
 * `getFieldRegistry(tenant)`: the cascade selects on `level`/`businessUnit`,
 * never on a label, and `tenant` here is only the bureau-tier gate. In the app
 * the two resolve the same tenant anyway. Call `getFieldRegistry(tenant)`
 * directly when you need another tenant's wording.
 */
