...
Three pieces of pure, unit-tested logic build on those fields:

- **Export** — `lib/ombExport.ts` maps a submission to one CSV row in the OMB inventory's column order (`app/api/export/omb/route.ts` streams it), including a derived "Reporting Mode" column: high-impact use cases are always mapped to `"Individual"` (never consolidated), everything else to `"Consolidated-eligible"` (`reportingMode()` — actual bureau-level consolidation of the eligible rows is a follow-on piece of work).
