...
- **`level` metadata** — `FieldDefinition.level` (`lib/fieldRegistry.ts`) is `"omb" | "department" | "bureau"`, defaulting to `"bureau"` when unset (`fieldLevel()`) so every pre-existing registry entry is unchanged unless explicitly marked. The OMB use-case-inventory fields are `level: "omb"` — reconciled against `docs/omb-2025-inventory-fields.md`'s exact 32-field list (23 base + 9 high-impact-only): every Free Text/Link/Date/Email/binary-Yes-No field from that list is represented (annotated `#N` in `lib/fieldRegistry.ts`), covering 27 of 32. The remaining 5 — `#4` Bureau/Component (has a stand-in, `submitterOffice`) and `#9`/`#10`/`#21`/`#32` — are Multiple Choice/Select-all-that-apply fields whose OMB-defined answer-option lists aren't in the extracted doc, so they're left unimplemented rather than guessed (see the note above the OMB block in `lib/fieldRegistry.ts`). The four DoC-mandated AI risk questions (`involvesSensitiveData`, `aiDecisionalImpact`, `aiModelSourcing`, `aiHumanReview` — already `locked: true` before this change) are `level: "department"`, not `"omb"` — a distinct DoC/executive-order mandate, not one of the 32 OMB inventory fields itself. A `businessUnit` field optionally scopes a `level: "bureau"` field to one bureau (e.g. a future Census-only optional field) — no such field exists yet, but the mechanism supports it: adding one is a registry entry, not a code change.
...
- **Remaining gap (issue #60)** — 4 of the 32 OMB inventory fields (Use Case Topic Area `#9`, AI Classification `#10`, demographic variables `#21`, end-user/public feedback consultation `#32`) have no registry entry yet: they're Multiple Choice/Select-all-that-apply fields, and `docs/omb-2025-inventory-fields.md` gives field name + data type, not OMB's actual pick-list values — inventing option lists for federal compliance reporting risks shipping wrong values, so they're deferred rather than guessed. The conditional-disclosure mechanism they'd use (`showWhen`, above) is already built and wired onto every field that exists today, including the newly-added OMB sub-fields (`atoSystemName`, `systemSourceVendorName`, `openSourceCodeLink`) and the three added high-impact-only fields (`independentReviewConducted`, `operatorTrainingEstablished`, `failSafeMechanism`).
