...
- **2026-06-29 — Seed: spread statuses + submitter-owned records.** Two issues: (1) seeded submissions all stacked in the pipeline's "Submitted" column, and (2) "My ideas" was empty for the demo submitter. Cause of (1): `getStatus()` prefers the top-level `status` column; `POST /api/submissions` syncs that column from `form_data.reviewStatus`, but `POST /api/seed` did not — so seeded rows took the DB default (`"submitted"`). Fix: the seed route now best-effort syncs `status` / `owner_email` / `business_unit` from each `form_data` after insert (mirrors the submissions POST). For (2): added three records owned by `submitter@uspto.gov` ("USPTO Submitter") in `lib/seedSubmissions.ts` — *Trademark Application Status Assistant* (approved/completed), *Automated TEAS Submission Validator* (needs_info, with a reviewer comment), *Trademark Conflict Search Advisory Assistant* (in_review) — so "My ideas" shows a real spread including a completed example. Note: `/api/seed` only inserts when the table is empty; the admin **Reload Demo Submissions** button clears first, so it always applies the current seed set.
