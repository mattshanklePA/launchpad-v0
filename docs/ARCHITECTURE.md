...
The wizard captures OMB M-25-21 companion-guidance fields on Step 7 (Feasibility & Security) — `stageOfDevelopment`, `highImpactFactors`, `highImpact`, `hasATO`, `systemSource`, `nationalSecuritySystem`, `researchOnly`, `useCaseTopicArea`, `aiClassification` (`lib/steps.ts`), each registered in `lib/fieldRegistry.ts` with `omb: true` (renders an `OmbBadge`, `components/launchpad/omb-badge.tsx`, next to the field). All are optional, so USPTO/DoW tenants that don't fill them in are unaffected. `useCaseTopicArea` and `aiClassification` are AI-proposed rather than typed from scratch — see the auto-fill section below.
...
#### Cutting OMB intake burden: auto-fill and AI-proposed fields ([issue #61](https://github.com/mattshanklePA/launchpad-v0/issues/61))

With the OMB field set loaded (previous section), a submitter should almost never author all of it by hand — most of it is already known from their profile, the wizard they fill anyway, or determinations the app already computes. `lib/ombAutofill.ts` is the single module all three mechanisms live behind, each pure/no I/O so they're unit-tested without a session or Supabase connection:

- **Profile auto-fill (zero typing)** — `profileAutofill(session)` maps the logged-in user's session straight onto `submitterName`/`submitterEmail`/`submitterRole`/`submitterOffice` (bureau/component) /`submitterSubOffice` (office). Agency itself needs no field — it's implicit in the active tenant. `context/form-context.tsx`'s `profileFromSession()` just supplies `getSession()` to it, so a fresh draft (and a fresh draft after a submission) starts with Step 1 already filled in and the submitter dropped straight to Step 2 — this predates issue #61, `profileAutofill` just extracted the existing mapping into a testable pure function.
- **Wizard-answer reuse (no double entry)** — several OMB inventory questions are answered by the *same* field the wizard already collects for its own purpose (`coreProblem` → "What problem is the AI intended to solve?", `solutionSummary` → "Describe the AI system's outputs", `businessValue` → "Expected benefits", `involvesSensitiveData` → "Involves PII?", `aiModelSourcing`/`systemSource` → sourcing). There's no separate OMB copy of these questions to fill twice — `WIZARD_TO_OMB_FIELD_MAP`/`getWizardDerivedOmbFields()` documents the mapping explicitly (mirrors `lib/ombExport.ts`'s CSV column mapping, which reads the same fields) so it's visible and testable rather than implicit.
- **AI-proposed, submitter-confirmed fields (Kestrel)** — `proposeHighImpact` (wraps `lib/highImpactDetermination.ts`), `proposeUseCaseTopicArea` (new `lib/useCaseTopicArea.ts` — deterministic keyword matching against problem/solution text, same pattern as `lib/ombConsolidation.ts`; a general-purpose taxonomy, not a verbatim OMB transcription — see the TODO in that file), `proposeAiClassification` (rights-impacting / safety-impacting / both / not classified, derived from `lib/highImpactDetermination.ts`'s `allHighImpactFactors()` — the same manual-plus-inferred factor set the high-impact recommendation itself uses, so the two never disagree), `proposeConsolidation` (wraps `lib/ombConsolidation.ts`), and `proposePublicIndicator` (defaults to `"public"` unless PII, a Controlled classification, or an NSS/IC flag is already on the form) each return `{ value, rationale }`. The wizard steps (`components/steps/step-8-feasibility-security.tsx` for `highImpact`/`aiClassification`/`useCaseTopicArea`/the consolidation override, `step-2-use-case-overview.tsx` for `publicIndicator`) pre-fill the underlying field the first time it's empty and render `<AiProposedHint>` (`components/launchpad/ai-proposed-hint.tsx`) alongside it — the rationale plus a "not sure? clear it" override; leaving the pre-filled value in place *is* the confirm. **Never fabricates**: an empty `value` (e.g. `proposeUseCaseTopicArea` finding no keyword match) means the field stays blank and shows up in "still needed" below instead of guessing.
- **Consolidation override** — `lib/ombConsolidation.ts`'s `resolveConsolidation(fd)` layers a reviewer's `consolidationOverride` (`"individual" | "consolidated" | ""`) on top of `determineConsolidation()`'s automatic call — except a high-impact use case, which OMB M-25-21 Section 5 always requires be reported individually regardless of any override, and a `"consolidated"` override is a no-op unless there's actually a category match to consolidate under. `lib/ombExport.ts`'s CSV export calls `resolveConsolidation()` (not `determineConsolidation()` directly) so an override actually changes what gets reported, not just what the wizard displays.
- **Required only at submit** — `lib/submissionReadiness.ts` (Issue A's resolver) now also checks the OMB fields (`stageOfDevelopment`, `highImpact`, `hasATO`, `systemSource`, `nationalSecuritySystem`, `researchOnly`, `useCaseTopicArea`, `aiClassification`), with the M-25-21 high-impact minimum-practice fields (`aiImpactAssessment`, `preDeploymentTesting`, `ongoingMonitoringPlan`, `humanOversightAppeal`) gated on `highImpact === "yes"` via a `need(..., applicable)` flag — an inapplicable field is skipped exactly like a disabled one, so a non-high-impact draft's "still needed" list never includes them. `totalChecks` is now just `checkedFields.length + 1` (every field `need()` actually ran, built up as it runs) instead of a separately-maintained static key list, so a future check can't silently drift out of the completeness denominator.
...
- **`level` metadata** — `FieldDefinition.level` (`lib/fieldRegistry.ts`) is `"omb" | "department" | "bureau"`, defaulting to `"bureau"` when unset (`fieldLevel()`) so every pre-existing registry entry is unchanged unless explicitly marked. The current OMB use-case-inventory fields (`stageOfDevelopment`, `highImpact`, `hasATO`, `systemSource`, `nationalSecuritySystem`, `researchOnly`, `highImpactFactors`, and the four M-25-21 high-impact risk-management fields) are `level: "omb"`; the four DoC-mandated AI risk questions (`involvesSensitiveData`, `aiDecisionalImpact`, `aiModelSourcing`, `aiHumanReview` — already `locked: true` before this change) are `level: "department"`. A `businessUnit` field optionally scopes a `level: "bureau"` field to one bureau (e.g. a future Census-only optional field) — no such field exists yet (see the TODO below), but the mechanism supports it: adding one is a registry entry, not a code change.
...
- **TODO** — the OMB `level` assignment above reuses the pre-cascade, hand-curated subset of OMB inventory fields already in the registry; it is not yet reconciled against the authoritative current-year field list in `context/Guidance-on-2025-Agency-Artificial-Intelligence-Reporting-.pdf` and `context/OMB AI Inventory Reporting Cheat Sheet - 12-5-25.docx`, which this environment couldn't parse (no `pdftotext`/`poppler-utils` for the PDF; docx extraction needs shell tooling this sandbox didn't have approval to run). See the TODO comment directly above the OMB block in `lib/fieldRegistry.ts` for what to do once those docs can be read.
