...
#### Command Center landing dashboard: scope + metrics

`lib/dashboard/` is the start of a landing dashboard (the "Command Center") that gives a viewer a summary view scoped to their place in the org, instead of landing directly on the full pipeline. Two pure, I/O-free modules, no new data model — everything is derived from the same libs the pipeline/roll-up/OMB surfaces already use:

- **`lib/dashboard/scope.ts`** — resolves which slice of the org hierarchy a viewer's dashboard should summarize. `getDashboardScope(session)` returns a `DashboardScope` (`"department" | "bureau" | "office" | "personal"`, carrying `businessUnit`/`office`/`email` as applicable), computed with the *exact same roll-down rule* `visibleSubmissions` (`lib/reviewWorkflow.ts`) already applies: a submitter is `"personal"`; a reviewer, or an admin whose `businessUnit` isn't `"os"`, is `"bureau"`-scoped (`"office"`-scoped once `office` is set); the Office of the Secretary and any viewer with no `businessUnit` (a department admin) get `"department"`. `getHierarchy()` exposes the full department → bureau → office tree from `getTenant().unit.options`; `getScopeSubtree()` returns the bureau/office constraint a scope implies (`[]` for `"department"`/`"personal"`); `isLevelAllowed(scope, level)` says whether a scope is broad enough to render a card aggregated at a given level (department is broadest, personal narrowest). Like `visibleSubmissions`, this is a view-model convenience, not a security boundary.
- **`lib/dashboard/metrics.ts`** — pure functions that take a `DashboardScope` plus the app's cached submissions and return typed Command Center card values, reusing rather than reimplementing each domain module: pipeline status counts (`getStatus`/`STATUS_ORDER`, `lib/reviewWorkflow.ts`), a readiness-score distribution (`formData.readinessScore`), a recommended-high-impact count (`determineHighImpact`, `lib/highImpactDetermination.ts` — the manual-plus-inferred recommendation, not the bare self-reported flag), an awaiting-bureau-sign-off count (`getBureauSignoff`, `lib/bureauSignoff.ts` — the "approved but not yet signed off" gap, always 0 for a tenant with no bureau tier), an OMB reportability + consolidated/individual breakdown (`determineReportability`/`determineConsolidation`), and a cross-bureau duplicate-cluster summary (`clusterDuplicates`/`isRationalizationPending`, `lib/rationalization.ts` — clustering deliberately runs over the *full* unscoped submission set per that module's own rule, then is filtered down to clusters touching the scope). Per-bureau roll-up rows are built the same way `components/admin/bureau-rollup.tsx` aggregates (one row per bureau present in scope, via `getStatus`/`getBusinessUnit`/`STATUS_ORDER`); per-office roll-up rows for the scope's own bureau delegate directly to `officeRollupRows` (`lib/officeRollup.ts`). Every card is built on top of `scopedSubmissions(scope, submissions)`, which mirrors `visibleSubmissions`'s filter predicate keyed off a `DashboardScope` instead of a `{ role, businessUnit, office }` viewer — so a bureau/office-scoped card can never count a submission outside that scope. `getDashboardMetrics(scope, submissions)` computes every card in one pass.
- **`components/dashboard/`** — the presentational shell CC-1/CC-2 render into (CC-3). Every component takes typed props and does no fetching: `kpi-card.tsx` (`KpiCard`/`KpiCardGrid`, a generic `label`/`value`/`delta`/`status` grid), `health-gauge.tsx` (`HealthGauge`, a single 0-100 scalar rendered as a color-banded semicircular arc), `entity-tree.tsx` (`EntityTree`, a lightweight custom Department → Bureau → Office nav driven by `getHierarchy()`'s `OrgHierarchy` — no `react-arborist`; a bureau with no `offices` renders as a leaf, so USPTO/DoW show bureaus only while DoC expands into offices), `charts/pipeline-status-chart.tsx` and `charts/readiness-distribution-chart.tsx` (recharts panels typed directly against `PipelineStatusCard`/`ReadinessDistributionCard` from `lib/dashboard/metrics.ts`, using the app's existing `--chart-*` CSS variables so they follow light/dark), and `action-center.tsx` (`ActionCenter`, an `ActionItem[]` list with a button per item — `onAction` is optional and a button without one renders disabled; CC-5, below, wires real handlers into some of them). Each component's non-trivial pure logic (band thresholds, severity ordering, selection matching, card → chart-row mapping) is split into a sibling `*-data.ts` module with its own unit tests, the same pattern `lib/officeRollup.ts` / `components/admin/office-rollup.tsx` already established, so the logic is tested without rendering React. Nothing imports these yet outside a throwaway dev harness (`app/dev/command-center-preview/page.tsx`, not linked from any nav) — wiring them into the real Department view is CC-4.

#### Department Dashboard — the first assembled Command Center screen (CC-4)

`components/dashboard/department-dashboard.tsx` (`DepartmentDashboard`) assembles the CC-3 shell against CC-2 metrics for a department-scoped admin, mounted at a temporary admin-gated preview route, `app/dashboard/page.tsx` (`RequireAuth requireRole={["admin"]}`, plus a `getDashboardScope(session).level === "department"` check that redirects a bureau/office-scoped admin or any non-admin to `/home`). Mostly read-only insight and navigation — the Executive Action Center is now live (CC-5, below). This does **not** repoint `/home` — that cutover is CC-6, and `app/admin` is untouched.

- **Assembly, not new logic** — the component resolves the viewer's base scope via `getDashboardScope(session)`, computes `getDashboardMetrics(scope, submissions, tenant)` for the KPI row/charts/health gauge/Action Center, and passes the scope's submissions (`scopedSubmissions`) straight into the existing `BureauRollup` (which itself renders `OfficeRollup` on expand) rather than a new roll-up table — so the same tenant-aware bureau/office rendering, sign-off column, and OMB/duplicate columns `components/home/reviewer-home.tsx`'s Pipeline page already has show up here unchanged.
- **Drill-down is local state** — selecting a bureau (then an office where the tenant has one) in `EntityTree` updates a local `EntitySelection`; `resolveDrillScope` (`components/dashboard/department-dashboard-data.ts`) turns that into a narrower `DashboardScope` fed back through the same `getDashboardMetrics` call, so every card — KPIs, charts, health gauge, Action Center, and the roll-up — recomputes for the selected node with no separate per-card fetch. A selection outside what the viewer's base scope is broad enough to see (`isLevelAllowed`) is ignored, so tree selection can never be used to escape the viewer's own roll-down.
- **Tenant-driven rendering** — the sign-off and cross-bureau-duplicate KPI/Action-Center items only appear for bureau-tier tenants (`tenantHasBureauTier`, DoC), matching every other bureau-tier-gated surface; pipeline size, readiness, recommended high-impact, and OMB reportability (with its consolidated/individual split) render for every tenant, including USPTO where `EntityTree` shows bureaus only (no office expansion).
- **Pure logic split out for unit tests** — `department-dashboard-data.ts` holds `resolveDrillScope`, `scopeLabel` (the header's current-scope breadcrumb), `buildKpiCards`, `buildActionItems`, and `computeHealthScore` (a 0-100 scalar weighting ready submissions fully and needs-work submissions at half), tested in `department-dashboard-data.test.ts` without rendering React — the same split every other `components/dashboard/*` piece uses.

#### Executive Action Center wired live (CC-5)

CC-3/CC-4 rendered the Action Center with real counts but inert buttons. `lib/dashboard/actions.ts` (`getDashboardActions`) derives real, per-submission action items — no new data model, built on the same modules the pipeline/roll-up already use (`lib/reviewWorkflow.ts`, `lib/bureauSignoff.ts`) and scoped via CC-2's `scopedSubmissions`, so a bureau/office-scoped reviewer's action list can never include another bureau's item, matching every other Command Center card:

- **`needs_info`** — submissions waiting on a reply from the submitter.
- **`unassigned`** — active (submitted/in-review/needs-info) submissions with no reviewer on file (`getAssigneeEmail`).
- **`signoff_nudge`** — approved submissions with no recorded `bureauSignoff` (`lib/bureauSignoff.ts`), bureau-tier tenants only (`tenantHasBureauTier`), matching `awaitingSignoff`'s own predicate in `lib/dashboard/metrics.ts`.

**The recipient guard (`canAutoSend`)** — the app has never had a channel to notify a submitter outside the product itself (no email/Teams adapter is wired up; the Slack webhook posts to one internal team channel). This story does not invent one, and preserves that boundary exactly: a `needs_info` action is aimed at the submitter, so `canAutoSend` is always `false` — reaching them still goes through the existing, guarded in-app path (Request info → `addSubmissionComment`, `components/submissions/submission-detail.tsx`). An `unassigned` action has no reviewer on file to address at all, so it's always `false` too, until an admin assigns one (`components/admin/user-management.tsx`). A `signoff_nudge` action targets the bureau's own on-file reviewer (the submission's assignee, or `assigneeForBusinessUnit` — the same lookup `lib/adapters/default/supabaseStore.ts` uses to auto-assign a new submission) — a known internal recipient on the same Slack channel every other internal notification already uses — so `canAutoSend` is `true` only when one is actually on file. `buildNotificationBatch(actions)` (also `lib/dashboard/actions.ts`) joins only the `canAutoSend` messages from a batch and returns `null` when nothing in it may auto-send, so a caller can tell "drafted, not sent" apart from "sent to N" without re-deriving the guard itself.

**Wiring, not a new send path** — `app/dashboard/dashboard-actions.ts` (`sendDashboardActionNotification`, a server action, same pattern as `app/admin/compare-actions.ts`) calls `buildNotificationBatch` and, only if it returns non-null, `getNotifier().send(...)` — the existing `Notifier` port (`lib/notifier.ts` → `lib/adapters/default/slackNotifier.ts` → `lib/slackWebhook.ts`). `department-dashboard-data.ts`'s `buildActionItems` now takes an optional third `dashboardActions` argument (defaulting to `[]`, so every existing call site and test is unaffected) and wires two `ActionItem`s to this same server action — **sign-off nudge** and **request info** — plus a third, informational-only **unassigned** item with no button. Both wired items call the identical `sendDashboardActionNotification`; the guard baked into each `DashboardAction` at derivation time is what actually decides whether Slack fires, not the UI layer.
- **Optimistic UI** — `ActionCenter` (`components/dashboard/action-center.tsx`) now supports `ActionItem.onAction: () => Promise<ActionOutcome>` (`ActionOutcome = { status: "sent" | "drafted"; description? }`, `action-center-data.ts`): a per-item pending spinner while the action resolves, then a toast — "Notification sent" for `"sent"`, "Drafted, not sent" for `"drafted"` (both success-styled; a thrown error is the only failure/destructive toast) — the same async-button + toast pattern as the Reset Demo Data button (`app/admin/page.tsx`).
