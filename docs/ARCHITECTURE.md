...
#### Duplicate rationalization gate — intra-bureau and cross-bureau ("one Commerce")

Cross-bureau duplicate *detection* (`lib/similarity.ts` + `lib/crossBureauDuplicates.ts`, the "Possible duplicates" roll-up column and the submission detail "Similar use cases" panel) was purely informational — nothing made anyone act on "~20 of the same thing across the bureaus," the top concern from the DoC demo. `lib/rationalization.ts` adds the gate on top ([issue #52](https://github.com/mattshanklePA/launchpad-v0/issues/52)), originally cross-bureau only; [issue #68](https://github.com/mattshanklePA/launchpad-v0/issues/68) scoped detection to the viewer's own visibility so a bureau reviewer catches duplicated effort **within their own bureau** too, not just across bureaus:

- **The edge rule is bureau-agnostic** — `lib/crossBureauDuplicates.ts`'s `isDuplicatePair(a, b)` (renamed from `isCrossBureauDuplicatePair`) no longer requires `a` and `b` to be filed under different bureaus. It keeps the other two rules exactly as before: `ROLLUP_DUPLICATE_THRESHOLD` = 0.25 (`lib/similarity.ts` score at or above threshold) and the same-`lib/ombConsolidation.ts`-category exclusion (those are consolidatable-by-design commercial AI, not wasteful duplicates). `hasCrossBureauMatch`/`crossBureauDuplicateCount` — the roll-up's separate "Possible duplicates" column — are unchanged and still cross-bureau only.
- **Clustering follows the viewer's visibility, with no new scoping logic** — `clusterDuplicates(submissions, tenant)` groups `submissions` into connected components of the `isDuplicatePair` graph and returns a cluster once it has 2+ members (the old "must span 2+ bureaus" requirement is gone). Every cluster carries `scope: "intra_bureau" | "cross_bureau"`, derived from whether its members land in one bureau or more than one. Callers pass `visibleSubmissions(all, viewer)` (`lib/reviewWorkflow.ts`) instead of the full list — since a bureau-scoped reviewer's visible set never contains another bureau's submissions, only `intra_bureau` clusters can ever form for them; a department-level viewer's visible set is unrestricted, so both scopes form. Cluster ids stay deterministic (lexicographically smallest member submission id). No-op (`[]`) for tenants without a bureau tier: `tenantHasBureauTier()` checks whether any `unit.options` entry declares `offices` (DoC-only, same signal `lib/tenant/index.ts`'s `getOrgNameForUnit` uses), so USPTO/DoW never see a cluster or a gate.
- **Decision authority is scoped to cluster scope** — `canRationalizeCluster(cluster, viewer)` is the predicate: an `intra_bureau` cluster is decidable by a reviewer/admin of that bureau, or by any department-level viewer (`isDepartmentLevelViewer`, `lib/fieldRegistry.ts` — an `os`-bureau or no-bureau-assignment viewer); a `cross_bureau` cluster is decidable ONLY by a department-level viewer, so a bureau reviewer can never consolidate away another bureau's use case. Whoever has authority marks the cluster **consolidated** (picks a lead submission; the rest point at it) or **keep-separate** (not real duplicates / intentionally parallel) in the rationalization panel. The decision is persisted as `form_data.rationalization = { clusterId, decision, leadSubmissionId?, decidedBy, decidedAt }` on **every** cluster member (`lib/reviewWorkflow.ts`'s form-data-as-storage pattern — no schema change), written via `patchSubmissionFormData`. `lib/rationalization.ts` itself stays pure/I/O-free; `buildRationalizationPatch()` builds the patch object and callers apply it.
- **The gate blocks intra-bureau clusters too** — `isRationalizationPending()`/`canApprove()`/`rationalizationBlockReason()` are the predicate: a submission that's a cluster member with no decision (or a decision recorded against a stale cluster id) is pending and blocks approval, `intra_bureau` clusters included — a bureau reviewer clears their own bureau's clusters themselves before any member can be approved. A non-member is always approvable. Wired into the only reviewer approve path, `setSubmissionStatus(id, "approved")` in `components/submissions/submission-detail.tsx` — the Approve button is disabled with the reason shown inline, and `setStatus()` re-checks the gate before calling `setSubmissionStatus` so nothing else can bypass it. Deliberately computed against the **full, unscoped** submission set (not the viewer's `visibleSubmissions` roll-down that the "Similar use cases" panel uses) — a bureau-scoped reviewer approving their half of a duplicate must still be blocked even when they can't see the other half (a cross-bureau cluster) from their own view.
- **Surfacing** — `components/admin/rationalization-panel.tsx`, rendered on the Pipeline page (`components/home/reviewer-home.tsx`) below the bureau roll-up, lists each cluster's members (title, bureau, link), a scope badge (Intra-bureau / Cross-bureau), and similarity. It shows the decision control (pick-a-lead + Consolidate/Keep-separate buttons) only when `canRationalizeCluster` allows the current viewer to decide that cluster — otherwise a note explains only a department-level reviewer can — and `decide()` re-checks the same predicate before writing, so the action path can't be bypassed either. Receives the same viewer-scoped `submissions` prop as `BureauRollup`, so scope naturally falls out of `visibleSubmissions`: a bureau reviewer sees only their bureau's `intra_bureau` clusters, a department/OS viewer sees both scopes. The submission detail page also shows a compact per-submission cluster card (pending vs. decided, scope-aware wording, with the other members) next to the Approve button.
- **Roll-up** — `components/admin/bureau-rollup.tsx` keeps the existing cross-bureau "Possible duplicates" column and adds an "Intra-bureau duplicates" column (gated on `tenantHasBureauTier()` like "Signed off"): per bureau, the count of undecided `intra_bureau` clusters (`pendingIntraBureauClusterCount()`), computed from the same viewer-scoped `submissions` the roll-up already receives — so a department admin sees, at a glance, that (say) NOAA has N unresolved internal duplicate clusters.

**Duplicate-rationalization vs. OMB-consolidation — two different axes, kept separate:** this gate (the *duplicate* axis) asks "is this the same effort as something else — in this bureau or another — and should we stop funding it twice?" `lib/ombConsolidation.ts`'s `determineConsolidation()` (the *OMB category* axis, previous section) asks a different question of a single submission — "does this match an OMB widely-used-commercial-AI category that can be reported once instead of per-bureau?" A submission can be in a rationalization cluster and independently be OMB-`Consolidated` or -`Individual`; the two never merge into one control, and `crossBureauDuplicates.ts`'s same-category exclusion keeps them from double-flagging the same pair in practice.
