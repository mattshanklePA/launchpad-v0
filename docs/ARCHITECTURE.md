...
- **TODO (still open as of issue #60)** — the OMB `level` assignment above reuses the pre-cascade, hand-curated subset of OMB inventory fields already in the registry (23 base + 9 high-impact-only per issue #60, not yet reconciled to the full 32). #60 asked to reconcile against `docs/omb-2025-inventory-fields.md`, but that file doesn't exist in this repo; the only source material is still `context/Guidance-on-2025-Agency-Artificial-Intelligence-Reporting-.pdf` and `context/OMB AI Inventory Reporting Cheat Sheet - 12-5-25.docx`, and this environment still can't parse either (no `pdftotext`/`poppler-utils` for the PDF; docx extraction needs shell tooling — python3/unzip — this sandbox doesn't have approval to run). Unblocking needs either the authoritative field list committed as `docs/omb-2025-inventory-fields.md`, or that tooling approval so a future run can extract the source text itself. See the TODO comment directly above the OMB block in `lib/fieldRegistry.ts`. The conditional-disclosure mechanism this reconciliation would plug into (`showWhen`, above) is already built and wired onto the fields that exist today.
